    ...
    public partial class SpreadingDevastationModSystem
    {
        ...
        private void RegisterDevastateCommand(ICoreServerAPI api, string commandName)
        {
            api.ChatCommands.Create(commandName)
                ...
                .EndSubCommand()
                .BeginSubCommand("wall")
                    .WithDescription("Configure fog walls at devastated chunk boundaries")
                    .WithArgs(api.ChatCommands.Parsers.OptionalWord("setting"),
                              api.ChatCommands.Parsers.OptionalAll("value"))
                    .HandleWith(HandleFogWallCommand)
                .EndSubCommand();
        }
        ...
        private TextCommandResult HandleFogWallCommand(TextCommandCallingArgs args)
        {
            string setting = (args.Parsers[0].GetValue() as string ?? "").ToLowerInvariant();
            string value = args.Parsers[1].GetValue() as string ?? "";
            ...
            switch (setting)
            {
                ...
                case "off":
                    if (setting == "on" || (setting == "enabled" && value.ToLowerInvariant() == "on"))
                    {
                        config.FogWallsEnabled = true;
                        SaveConfig();
                        BroadcastFogWallConfig();
                        return TextCommandResult.Success("Fog walls ENABLED");
                    }
                    else if (setting == "off" || (setting == "enabled" && value.ToLowerInvariant() == "off"))
                    {
                        config.FogWallsEnabled = false;
                        SaveConfig();
                        BroadcastFogWallConfig();
                        return TextCommandResult.Success("Fog walls DISABLED");
                    }
                    return TextCommandResult.Error("Usage: /dv wall [on|off]");

                case "height":
                    if (string.IsNullOrWhiteSpace(value))
                    {
                        return TextCommandResult.Success($"Fog wall height: {config.FogWallHeight:F0} blocks. Use '/dv wall height [blocks]' to set.");
                    }
                    if (float.TryParse(value, NumberStyles.Float, CultureInfo.InvariantCulture, out float height))
                    {
                        config.FogWallHeight = Math.Clamp(height, 1f, 500f);
                        SaveConfig();
                        BroadcastFogWallConfig();
                        return TextCommandResult.Success($"Fog wall height set to {config.FogWallHeight:F0} blocks");
                    }
                ...
                case "alpha":
                    if (string.IsNullOrWhiteSpace(value))
                    {
                        return TextCommandResult.Success($"Fog wall opacity: {config.FogWallOpacity:F2}. Use '/dv wall opacity [0.0-1.0]' to set.");
                    }
                    if (float.TryParse(value, NumberStyles.Float, CultureInfo.InvariantCulture, out float opacity))
                    {
                        config.FogWallOpacity = Math.Clamp(opacity, 0f, 1f);
                        SaveConfig();
                        BroadcastFogWallConfig();
                        return TextCommandResult.Success($"Fog wall opacity set to {config.FogWallOpacity:F2}");
                    }
                    return TextCommandResult.Error("Invalid number for opacity");

                case "color":
                    if (string.IsNullOrWhiteSpace(value))
                    {
                        return TextCommandResult.Success($"Fog wall color: R={config.FogWallColorR:F2} G={config.FogWallColorG:F2} B={config.FogWallColorB:F2}. Use '/dv wall color [r] [g] [b]' (0.0-1.0)");
                    }
                    var parts = value.Split(' ', StringSplitOptions.RemoveEmptyEntries);
                    if (parts.Length >= 3 &&
                        float.TryParse(parts[0], NumberStyles.Float, CultureInfo.InvariantCulture, out float r) &&
                    ...
                    return TextCommandResult.Error("Usage: /dv wall color [r] [g] [b] (values 0.0-1.0, e.g., '0.55 0.25 0.15')");

                case "fade":
                case "fadedistance":
                    if (string.IsNullOrWhiteSpace(value))
                    {
                        return TextCommandResult.Success($"Fog wall fade distance: {config.FogWallFadeDistance:F0} blocks. Use '/dv wall fade [distance]' to set.");
                    }
                    if (float.TryParse(value, NumberStyles.Float, CultureInfo.InvariantCulture, out float fadeDist))
                    {
                        config.FogWallFadeDistance = Math.Clamp(fadeDist, 0f, config.FogWallMaxDistance);
                        SaveConfig();
                        BroadcastFogWallConfig();
                        return TextCommandResult.Success($"Fog wall fade distance set to {config.FogWallFadeDistance:F0} blocks");
                    }
                    return TextCommandResult.Error("Invalid number for fade distance");

                case "max":
                case "maxdistance":
                    if (string.IsNullOrWhiteSpace(value))
                    {
                        return TextCommandResult.Success($"Fog wall max render distance: {config.FogWallMaxDistance:F0} blocks. Use '/dv wall max [distance]' to set.");
                    }
                    if (float.TryParse(value, NumberStyles.Float, CultureInfo.InvariantCulture, out float maxDist))
                    {
                        config.FogWallMaxDistance = Math.Clamp(maxDist, 100f, 5000f);
                        SaveConfig();
                        BroadcastFogWallConfig();
                        return TextCommandResult.Success($"Fog wall max render distance set to {config.FogWallMaxDistance:F0} blocks");
                    }
                    return TextCommandResult.Error("Invalid number for max distance");

                case "offset":
                case "zoffset":
                    if (string.IsNullOrWhiteSpace(value))
                    {
                        return TextCommandResult.Success($"Fog wall Z-offset: {config.FogWallZOffset:F2}. Use '/dv wall offset [value]' to set.");
                    }
                    if (float.TryParse(value, NumberStyles.Float, CultureInfo.InvariantCulture, out float zOffset))
                    {
                        config.FogWallZOffset = Math.Clamp(zOffset, 0f, 10f);
                        SaveConfig();
                        BroadcastFogWallConfig();
                        return TextCommandResult.Success($"Fog wall Z-offset set to {config.FogWallZOffset:F2}");
                    }
                    return TextCommandResult.Error("Invalid number for Z-offset");

                case "reset":
                case "defaults":
                    config.FogWallsEnabled = true;
                    config.FogWallHeight = 128f;
                    config.FogWallOpacity = 0.15f;
                    config.FogWallColorR = 0.55f;
                    config.FogWallColorG = 0.25f;
                    config.FogWallColorB = 0.15f;
                    config.FogWallMaxDistance = 1000f;
                    config.FogWallFadeDistance = 200f;
                    config.FogWallZOffset = 0.5f;
                    SaveConfig();
                    BroadcastFogWallConfig();
                ...
                default:
                    return SendChatLines(args, new[]
                    {
                        ...
                        $"Height: {config.FogWallHeight:F0} blocks",
                        $"Opacity: {config.FogWallOpacity:F2}",
                        $"Color (RGB): {config.FogWallColorR:F2}, {config.FogWallColorG:F2}, {config.FogWallColorB:F2}",
                        $"Fade distance: {config.FogWallFadeDistance:F0} blocks",
                        $"Max render distance: {config.FogWallMaxDistance:F0} blocks",
                        $"Z-offset: {config.FogWallZOffset:F2}",
                        "",
                        "Commands:",
                        "  /dv wall [on|off] - Enable or disable fog walls",
                        "  /dv wall height [blocks] - Set wall height",
                        "  /dv wall opacity [0.0-1.0] - Set wall opacity",
                        "  /dv wall color [r] [g] [b] - Set wall color (0.0-1.0)",
                        "  /dv wall fade [distance] - Set fade start distance",
                        "  /dv wall max [distance] - Set max render distance",
                        "  /dv wall offset [value] - Set Z-offset (anti z-fighting)",
                        "  /dv wall reset - Reset all wall settings to defaults"
